...
#if 0
	OutputMainChannel(fileMainInfo, lpInBuf + nOfs, NULL, nLBA, CD_RAW_SECTOR_SIZE);
#endif
	for (BYTE i = 0; i < byTransferLen; i++) {
...
BOOL ReadCDForRereadingSectorNew(
	PEXEC_TYPE pExecType,
	PEXT_ARG pExtArg,
	PDEVICE pDevice,
	PDISC pDisc,
	...
	try {
		...
		BYTE fixedMain[CD_RAW_SECTOR_SIZE] = {};
		BYTE tmpC2[CD_RAW_READ_C2_294_SIZE] = {};
		BYTE fixedC2[CD_RAW_READ_C2_294_SIZE] = {};

		for (INT m = 0; m < pDisc->MAIN.nC2ErrorCnt; m++) {
			ZeroMemory(fixedMain, CD_RAW_SECTOR_SIZE);
			FillMemory(fixedC2, CD_RAW_READ_C2_294_SIZE, 0xff);
			INT nLBA = pDisc->MAIN.lpAllLBAOfC2Error[m];

			for (UINT i = 0; i < pExtArg->uiMaxRereadNum; i++) {
				OutputLog(standardOut | fileC2Error, "\nNeed to reread sector: %6d", nLBA);
				...
				if (IsValid0xF1SupportedDrive(pDevice) || IsValidAsusDriveWith310(pDevice)) {
					memcpy(tmpC2, pDiscPerSector->data.current + CD_RAW_SECTOR_SIZE, CD_RAW_READ_C2_294_SIZE);
				}
				else {
					...
					if (1 <= pExtArg->uiSubAddionalNum && 0 < pExtArg->uiC2Offset && pExtArg->uiC2Offset < CD_RAW_READ_C2_294_SIZE) {
						memcpy(tmpC2, pDiscPerSector->data.current + CD_RAW_SECTOR_SIZE + pExtArg->uiC2Offset, CD_RAW_READ_C2_294_SIZE - pExtArg->uiC2Offset);
						memcpy(tmpC2, pDiscPerSector->data.next + CD_RAW_SECTOR_SIZE, pExtArg->uiC2Offset);
					}
					else if (1 <= pExtArg->uiSubAddionalNum && pExtArg->uiC2Offset == CD_RAW_READ_C2_294_SIZE) {
						memcpy(tmpC2, pDiscPerSector->data.next + CD_RAW_SECTOR_SIZE, CD_RAW_READ_C2_294_SIZE);
					}
						...
						UINT c2ofs = pExtArg->uiC2Offset - CD_RAW_READ_C2_294_SIZE;
						memcpy(tmpC2, pDiscPerSector->data.next + CD_RAW_SECTOR_SIZE + c2ofs, CD_RAW_READ_C2_294_SIZE - c2ofs);
						memcpy(tmpC2, pDiscPerSector->data.nextNext + CD_RAW_SECTOR_SIZE, c2ofs);
					}
					else {
						memcpy(tmpC2, pDiscPerSector->data.current + CD_RAW_SECTOR_SIZE, CD_RAW_READ_C2_294_SIZE);
					}
				...
				INT nC2ErrorBit = 0;
				for (INT t = 0; t < CD_RAW_READ_C2_294_SIZE; t++) {
					INT nBit = 0x80;
			...
			WriteMainChannel(pExecType, pExtArg, pDisc, fixedMain, nLBA, fpImg);

			LONG lSeekC2 = CD_RAW_READ_C2_294_SIZE * (LONG)nLBA - nStart;
			fseek(fpC2, lSeekC2, SEEK_SET);
			fwrite(fixedC2, sizeof(BYTE), CD_RAW_READ_C2_294_SIZE, fpC2);
		}
	}
	catch (BOOL bErr) {
		bRet = bErr;
...
BOOL ProcessDescramble(
	...
	if (IsAudioOnlyDisc(pDisc) || pDisc->SCSI.trkType & TRACK_TYPE::pregapAudioWithSyncIn1stTrack) {
		...
		SetLBAtoDescrambleSector(pDisc, pszPath);
		FILE* fpImg = NULL;
...
//		}
		...
		if ((pExtArg->byC2 || pExtArg->byC2New) && pDevice->FEATURE.byC2ErrorData) {
			if (bC2Error && pDisc->MAIN.nC2ErrorCnt > 0) {
				if (pExtArg->byC2New) {
					if (!ReadCDForRereadingSectorNew(pExecType, pExtArg, pDevice
						, pDisc, pDiscPerSector, CDFLAG::_READ_CD::CDDA, lpCmd, fpScm, fpC2, 0)) {
						throw FALSE;
					}
				}
				else if (pExtArg->nC2RereadingType == 0) {
		...
		if (!CreateCueCcd(pExtArg, pDisc, pszPath, pDevice->FEATURE.byCanCDText, fpCcd)) {
			throw FALSE;
		}
...
BOOL ReadCDPartial(
	...
	LPBYTE pNextNextBuf = NULL;
	INT nMainDataType = scrambled;
	if (*pExecType == data || pDisc->SCSI.trkType == TRACK_TYPE::audioTrack) {
		nMainDataType = unscrambled;
	}
	INT nPadType = padByUsr55;
	...
	try {
		...
		if (*pExecType == data && !pExtArg->byReverse) {
			SetCDOffset(pExecType, pDevice->byPlxtrDrive, pDisc, nStart, nEnd + 1);
		}
		else {
			SetCDOffset(pExecType, pDevice->byPlxtrDrive, pDisc, nStart, nEnd);
		}
...
//							if (nStart < nLBA) {
		...
		if ((pExtArg->byC2 || pExtArg->byC2New) && pDevice->FEATURE.byC2ErrorData) {
			if (bC2Error && pDisc->MAIN.nC2ErrorCnt > 0) {
				if (pExtArg->byC2New) {
					if (!ReadCDForRereadingSectorNew(pExecType, pExtArg, pDevice
						, pDisc, pDiscPerSector, CDFLAG::_READ_CD::CDDA, lpCmd, fpBin, fpC2, nStart)) {
						throw FALSE;
					}
				}
				else if (pExtArg->nC2RereadingType == 0) {
...
BOOL ReadCDForTages(
	...
	_TCHAR pszBinPath[_MAX_PATH] = {};
	FILE* fpBinForward = NULL;
	if (NULL == (fpBinForward = CreateOrOpenFile(pszPath, _T("_Forward"), pszBinPath, NULL, NULL, szExt, _T("wb"), 0, 0))) {
		OutputLastErrorNumAndString(_T(__FUNCTION__), __LINE__);
		return FALSE;
	}
	FILE* fpBinBack = NULL;
	if (NULL == (fpBinBack = CreateOrOpenFile(pszPath, _T("_Backward"), pszBinPath, NULL, NULL, szExt, _T("wb+"), 0, 0))) {
		OutputLastErrorNumAndString(_T(__FUNCTION__), __LINE__);
		return FALSE;
	}
	...
	LPBYTE pNextBuf = NULL;
	LPBYTE pNextNextBuf = NULL;
	INT nMainDataType = unscrambled;

	try {
		if (NULL == (fpSubForward = CreateOrOpenFile(
			pszPath, NULL, NULL, NULL, NULL, _T(".sub"), _T("wb"), 0, 0))) {
			OutputLastErrorNumAndString(_T(__FUNCTION__), __LINE__);
			throw FALSE;
		}
		if (NULL == (fpSubBack = CreateOrOpenFile(
			pszPath, NULL, NULL, NULL, NULL, _T(".sub"), _T("wb"), 0, 0))) {
			OutputLastErrorNumAndString(_T(__FUNCTION__), __LINE__);
			throw FALSE;
		}
...
#endif
		INT n1stLBA = nStart + pDisc->MAIN.nOffsetStart - 1;
		INT nLastLBA = nEnd + pDisc->MAIN.nOffsetEnd + 1;
		INT nLBA = n1stLBA;
		// init end
		FlushLog();
		...
		LPDWORD lpAllSectorCrc32 = NULL;
		LPINT lpBadSectorList = NULL;
		LPINT lpBadSectorLBA = NULL;
		size_t dwAllocSize = (size_t)(pDisc->SCSI.nAllLength + FIRST_TRACK_PREGAP_SIZE + LAST_TRACK_LEADOUT_SIZE);
		if (NULL == (lpAllSectorCrc32 = (LPDWORD)calloc(dwAllocSize, sizeof(DWORD)))) {
			OutputLastErrorNumAndString(_T(__FUNCTION__), __LINE__);
			return FALSE;
		}
		if (NULL == (lpBadSectorList = (LPINT)calloc(dwAllocSize, sizeof(INT)))) {
			OutputLastErrorNumAndString(_T(__FUNCTION__), __LINE__);
			return FALSE;
		}
		if (NULL == (lpBadSectorLBA = (LPINT)calloc(dwAllocSize, sizeof(INT)))) {
			OutputLastErrorNumAndString(_T(__FUNCTION__), __LINE__);
			return FALSE;
		}

		for (INT i = 0; i < 500; i++) {
			pDiscPerSector->bReturnCode = ProcessReadCD(pExecType, pExtArg, pDevice, pDisc, pDiscPerSector, lpCmd, nLBA - 500 + i);
		}
		while (n1stLBA < nLastLBA) {
			pDiscPerSector->bReturnCode = ProcessReadCD(pExecType, pExtArg, pDevice, pDisc, pDiscPerSector, lpCmd, nLBA);
			if (pDiscPerSector->bReturnCode == RETURNED_EXIST_C2_ERROR) {
				...
				pDiscPerSector->bReturnCode == RETURNED_FALSE) {
				FlushDriveCache(pExtArg, pDevice, nLBA - 1);
				Sleep(1000);
				OutputLog(standardError | fileMainError, "Retry %d/10", nRetryCnt);
				nRetryCnt++;
			...
			if (pDiscPerSector->bReturnCode != RETURNED_CONTINUE &&
				...
				GetCrc32(&lpAllSectorCrc32[n1stLBA - pDisc->MAIN.nOffsetStart]
					, pDiscPerSector->data.current, CD_RAW_SECTOR_SIZE);
				...
				// Write track to scrambled
				WriteMainChannel(pExecType, pExtArg, pDisc, pDiscPerSector->data.current, nLBA, fpBinForward);
#if 0
				OutputMainChannel(standardOut, pDiscPerSector->data.current, NULL, nLBA, 2352);
#endif
			}
		...
		if ((pExtArg->byC2 || pExtArg->byC2New) && pDevice->FEATURE.byC2ErrorData) {
			if (bC2Error && pDisc->MAIN.nC2ErrorCnt > 0) {
				if (pExtArg->byC2New) {
					if (!ReadCDForRereadingSectorNew(pExecType, pExtArg, pDevice
						, pDisc, pDiscPerSector, CDFLAG::_READ_CD::CDDA, lpCmd, fpBinForward, fpC2, nStart)) {
						throw FALSE;
					}
				}
				else if (pExtArg->nC2RereadingType == 0) {
					if (!ReadCDForRereadingSectorType1(pExecType, pExtArg, pDevice
						, pDisc, pDiscPerSector, type, lpCmd, fpBinForward, fpC2, nStart)) {
						throw FALSE;
					}
				}
				else {
					...
					if (!ReadCDForRereadingSectorType2(pExecType, pExtArg
						, pDevice, pDisc, type, lpCmd, fpBinForward, fpC2, nStartLBA, nEndLBA)) {
						throw FALSE;
					}
				}
			}
		...
		FcloseAndNull(fpBinForward);

		nLBA = nLastLBA;
		INT nBadSectorCnt = 0;
		INT nBadSectorNumIdx = 0;

		do {
			n1stLBA = nStart + pDisc->MAIN.nOffsetStart - 1;
			nLastLBA = nEnd + pDisc->MAIN.nOffsetEnd + 1;
			if (nBadSectorCnt > 0) {
				nLBA = lpBadSectorLBA[0];
			}
			nBadSectorCnt = 0;

			while (n1stLBA < nLastLBA) {
				FlushDriveCache(pExtArg, pDevice, nLBA + nRetryCnt);
				pDiscPerSector->bReturnCode = ProcessReadCD(pExecType, pExtArg, pDevice, pDisc, pDiscPerSector, lpCmd, nLBA);
				if (pDiscPerSector->bReturnCode == RETURNED_EXIST_C2_ERROR) {
					bC2Error = TRUE;
					// C2 error points the current LBA - 1 (offset?)
...
//					nRetryCnt++;
				...
				if (pDiscPerSector->bReturnCode != RETURNED_CONTINUE &&
					pDiscPerSector->bReturnCode != RETURNED_SKIP_LBA) {
					DWORD crc32 = 0;
					GetCrc32(&crc32, pDiscPerSector->data.current, CD_RAW_SECTOR_SIZE);

					if (crc32 == lpAllSectorCrc32[nLBA - pDisc->MAIN.nOffsetStart]) {
						BYTE m, s, f = 0;
						LBAtoMSF(nLBA + 150, &m, &s, &f);
						OutputLog(fileMainError, "LBA[%06d, %#07x][%02d:%02d:%02d] it's normal sector or failed to get the twin sector\n", nLBA, (UINT)nLBA, m, s, f);
						lpBadSectorList[nBadSectorCnt++] = nLBA;
//						OutputMainChannel(fileMainError, pDiscPerSector->data.current, "Bad twin sector", nLBA, CD_RAW_SECTOR_SIZE);
					}
					...
					// Write track to scrambled
					if (nRetryCnt != 1) {
						fseek(fpBinBack, (nEnd - nLBA) * CD_RAW_SECTOR_SIZE, SEEK_SET);
#if 0
						if (crc32 != lpAllSectorCrc32[nLBA - pDisc->MAIN.nOffsetStart]) {
							OutputMainChannel(fileMainInfo, pDiscPerSector->data.current, "Good twin sector", nLBA, CD_RAW_SECTOR_SIZE);
						}
#endif
					}
					WriteMainChannel(pExecType, pExtArg, pDisc, pDiscPerSector->data.current, nLBA, fpBinBack);
				}
				OutputString("\rCreating %s from %d to %d (LBA) %6d", szExt
					, nStart + pDisc->MAIN.nOffsetStart, nEnd + pDisc->MAIN.nOffsetEnd, nLBA);
				if (nRetryCnt == 1) {
					...
					nLBA = lpBadSectorLBA[++nBadSectorNumIdx];
					n1stLBA = nStart + nEnd - nLBA;
//					OutputLog(standardError | fileMainError, " nLBA: %d, n1stLBA: %d\n", nLBA, n1stLBA);
				}
			}
			OutputLog(standardError | fileMainError, " BadSectorCnt: %d, Retry %d/%d\n", nBadSectorCnt, nRetryCnt, pExtArg->uiMaxRereadNum);
			if (nRetryCnt++ == (INT)pExtArg->uiMaxRereadNum) {
				break;
			}
			memcpy(lpBadSectorLBA, lpBadSectorList, sizeof(INT) * dwAllocSize);
			ZeroMemory(lpBadSectorList, sizeof(DWORD)* dwAllocSize);
			nBadSectorNumIdx = 0;
		} while (nBadSectorCnt > 0);

		OutputString("\n");
		FcloseAndNull(fpSubBack);
		FlushLog();
		...
		FcloseAndNull(fpBinBack);

		FILE* fpBin_r = NULL;
		FILE* fpBin = NULL;
		if (NULL == (fpBin_r = CreateOrOpenFile(
			pszPath, _T("_Backward"), NULL, NULL, NULL, szExt, _T("rb"), 0, 0))) {
			OutputLastErrorNumAndString(_T(__FUNCTION__), __LINE__);
			throw FALSE;
		}
		if (NULL == (fpBin = CreateOrOpenFile(
			pszPath, _T("_BackToForward"), NULL, NULL, NULL, szExt, _T("wb"), 0, 0))) {
			OutputLastErrorNumAndString(_T(__FUNCTION__), __LINE__);
			throw FALSE;
		}
		OutputString("Reversing _Backward%s to _BackToForward%s\n", szExt, szExt);
		BYTE rBuf[CD_RAW_SECTOR_SIZE] = {};
		DWORD dwRoop = GetFileSize(0, fpBin_r) - CD_RAW_SECTOR_SIZE/* * 3*/;
		LONG lSeek = CD_RAW_SECTOR_SIZE - (LONG)pDisc->MAIN.uiMainDataSlideSize;
		fseek(fpBin_r, -lSeek, SEEK_END);
...
BOOL ReadCDOutOfRange(
	...
	if (pDisc->SCSI.by1stMultiSessionTrkNum) {
		...
		BOOL bPregap = FALSE;
		if (pDisc->SCSI.toc.TrackData[pDisc->SCSI.by1stMultiSessionTrkNum - 1].Control & AUDIO_DATA_TRACK) {
...
//					memcpy(lpSubcodeNext, pDiscPerSector->subcode.next, CD_RAW_READ_SUBCODE_SIZE);
	...
	// 1st session lead-in
	if (pDisc->SCSI.toc.TrackData[0].Control & AUDIO_DATA_TRACK ||
		pDisc->SUB.byCtlOfLBA0 & AUDIO_DATA_TRACK) {
		_tcsncpy(ext, _T(".scm"), SIZE_OF_ARRAY(ext));
	}
	else {
		_tcsncpy(ext, _T(".bin"), SIZE_OF_ARRAY(ext));
	...
	for (INT i = LEADIN_START_LBA_SESSION_1; i < -1153; i++) {
		...
		if ((lpSubcode[12] & 0x0f) == ADR_ENCODES_CURRENT_POSITION) {
			memcpy(lpSubcodeTmp, lpSubcode, CD_RAW_READ_SUBCODE_SIZE);
		}
		else {
			memcpy(lpSubcodeTmp, lpSubcodeNext, CD_RAW_READ_SUBCODE_SIZE);
		}
		if (lpSubcodeTmp[13] == 1 && (lpSubcodeTmp[14] == 0 || lpSubcodeTmp[14] == 1)) {
			...
			else if ((lpSubcodeTmp[19] == 0 && lpSubcodeTmp[20] == 1 && BcdToDec(lpSubcodeTmp[21]) == 71 && nOverreadSize == -4) ||
				(lpSubcodeTmp[19] == 0 && lpSubcodeTmp[20] == 1 && BcdToDec(lpSubcodeTmp[21]) == 72 && nOverreadSize == -3) ||
				(lpSubcodeTmp[19] == 0 && lpSubcodeTmp[20] == 1 && BcdToDec(lpSubcodeTmp[21]) == 73 && nOverreadSize == -2) ||
				(lpSubcodeTmp[19] == 0 && lpSubcodeTmp[20] == 1 && BcdToDec(lpSubcodeTmp[21]) == 74 && nOverreadSize == -1) ||
		...
		if (bLastPregapSector) {
			...
			if ((bTrack1 && lpSubcodeTmp[14] == 0) || !bTrack1) {
				fwrite(lpSubcode, sizeof(BYTE), CD_RAW_READ_SUBCODE_SIZE, fpSub);
